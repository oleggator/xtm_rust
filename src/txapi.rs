...
use flume::{self, TryRecvError};

type Task = Box<dyn FnOnce(&Lua) -> Result<(), ChannelError> + Send>;
type TaskSender = flume::Sender<Task>;
type TaskReceiver = flume::Receiver<Task>;
...
impl AsyncDispatcher {
            ...
            Func: Send + 'static,
    {
        ...
        if let Err(_channel_closed) = self.task_tx.send_async(handler_func).await {
...
pub fn channel(buffer: usize) -> io::Result<(Dispatcher, Executor)> {
    let (task_tx, task_rx) = flume::bounded(buffer);
